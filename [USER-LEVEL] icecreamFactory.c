...
void *iceCream(void *_id)
{
	...
	if(ticket <= 0)
	{
		//printf("\n[ICE CREAM FACTORY] Customer[%d]: Leaving Shop. [REASON]: Tickets Finished\n", _ID);
	...
	ticket--;
	//printf("[ICE CREAM FACTORY ]Customer[%d] Got Ticket.\n", _ID);
	...
	{	// printf("[ICE CREAM FACTORY] Customer[%d] Leaving Shop. [REASON]: Flavours Finished\n", _ID);
	...
	else
	{
		...
		if(_flavors[0] > 0)
		{
			...
			//printf("\n[ICE CREAM FACTORY ]Customer[%d]: Got Flavour[0].\n", _ID);
